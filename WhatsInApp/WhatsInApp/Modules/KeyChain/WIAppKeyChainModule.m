...
@implementation WIAppKeyChainModule
...
+ (id)loadContent
{
    WIAppKeyChainModule * sharedModule = [self sharedModule];
    [sharedModule loadContent];
    return sharedModule->allKeychainItems;
}
...
+ (void)displayInConsole
{
    WIAppKeyChainModule * sharedModule = [self sharedModule];
    [sharedModule loadContent];
    NSArray * kvModels = [WIAppKVUtil constructKVModel:sharedModule->allKeychainItems];
    NSLog(@"%@",kvModels);
}
...
- (void)loadContent
{
    ...
    if(!allKeychainItems[kstrKeyGenericPasswords]) allKeychainItems[kstrKeyGenericPasswords] = @[];
    if(!allKeychainItems[kstrKeyInternetPasswords]) allKeychainItems[kstrKeyInternetPasswords] = @[];
    if(!allKeychainItems[kstrKeyCertificates]) allKeychainItems[kstrKeyCertificates] = @[];
    if(!allKeychainItems[kstrKeyKeys]) allKeychainItems[kstrKeyKeys] = @[];
    if(!allKeychainItems[kstrKeyIdentities]) allKeychainItems[kstrKeyIdentities] = @[];
}
